...
buildah run $ctr -- yum install -y passwd rsync ansible strace make gcc python-jinja2 PyYAML git patch python-devel python-virtualenv curl wget curl-devel nodejs
## Install python3
buildah run $ctr -- yum install -y centos-release-scl-rh
buildah run $ctr -- yum install -y rh-python35-runtime rh-python35-PyYAML rh-python35-python rh-python35-python-devel rh-python35-python-lib rh-git29-git
...
echo "Push running:"
echo "sudo buildah push --creds=tdecacqu awx-test-image docker://docker.io/softwarefactoryproject/awx-test-image"
